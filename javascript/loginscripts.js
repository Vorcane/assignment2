        ...
        for (i = 0; i < cookiearray.length; i++) {
            ...
            if (cookie.indexOf(cookiename) === 0) { //If cookie exists do this
                ...
                $.post('checklogin.php', {sessionID: sessionID}, function (data) {
                    retdata = data.split(';');
                    if (retdata[0] === true) {
                        $('div#login-form').text("<h3>Welcome " + retdata[1] + "</h3>" + "<button onclick='deletecookie(\x22sessionID\x22)' type='button'>Log out</button>"); //removes the login forms and replaces it with a greeting message, logout button and link to user account page
                    }
                });
